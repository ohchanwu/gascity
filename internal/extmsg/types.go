...
//
// AgentName, when set instead of SessionID, defers session resolution to
// delivery time so the conversation survives session retirement and the
// agent's live session is resolved (and cold-woken) on demand. Exactly one
// of SessionID and AgentName is set on any given binding.
