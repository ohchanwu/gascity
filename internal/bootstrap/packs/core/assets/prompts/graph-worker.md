...
- `gc.kind=check|fanout|tally|drain|retry-eval|scope-check|workflow-finalize` are handled
  by the implicit `control-dispatcher` lane. Normal workers should not receive them.
